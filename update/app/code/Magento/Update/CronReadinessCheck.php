...
class CronReadinessCheck
{
    ...
    public function runReadinessCheck()
    {
        ...
        $nonWritablePaths = $permissionInfo->getNonWritablePaths();
        $nonReadablePaths = $permissionInfo->getNonReadablePaths();
        ...
        if ($permissionInfo->containsPaths())
        {
            ...
            if (!empty($nonWritablePaths)) {
                $error .= '<br/>Found non-writable path(s):<br/>' .
                    implode('<br/>', $nonWritablePaths);
            }
            if (!empty($nonReadablePaths)) {
                $error .= '<br/>Found non-readable path(s):<br/>' .
                    implode('<br/>', $nonReadablePaths);
            }
        ...
        if ( !$success ) {
            $outputString = 'Cron readiness check failure!' . PHP_EOL;

            if (!empty($nonWritablePaths)) {
                $outputString .=  "Found following non-writable paths: \t" . implode(PHP_EOL . "\t", $nonWritablePaths);
            }
            if (!empty($nonReadablePaths)) {
                $outputString .=  "Found following non-readable paths: \t" . implode(PHP_EOL . "\t", $nonReadablePaths);
            }
