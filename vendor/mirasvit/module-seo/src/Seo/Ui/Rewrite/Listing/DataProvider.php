...
namespace  Mirasvit\Seo\Ui\Rewrite\Listing;
...
class DataProvider extends \Magento\Framework\View\Element\UiComponent\DataProvider\DataProvider
{
    ...
    protected function searchResultToOutput(SearchResultInterface $searchResult)
    {
        ...
        if ($data = $searchResult->getData()) { //prepare store_id for multistore
            foreach ($data as $value) {
                $storeIds[$value['rewrite_id']] = $value['store_id'];
            }
        }
     ...
     * @return Mirasvit\Seo\Model\ResourceModel\Rewrite\Grid\Collection
     */
    protected function getPreparedCollection($fieldStoreValue)
    {
        ...
        if ($fieldStoreValue) {
            ...
            $collection->addStoreColumn()->getSelect()
                ->where(
                    new \Zend_Db_Expr('rewrite_id IN (' . implode(',', $linkIds) . ')')
                );
