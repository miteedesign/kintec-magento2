    ...
    public function execute()
    {
        ...
        if ($id != 0) {
            try {
                ...
                $data = $this->sgsHelper->reportToHtml($model->errorReport);
                $this->getResponse()->representJson($this->_objectManager->create('Magento\Framework\Json\Helper\Data')->jsonEncode($data));
            } catch (\Exception $e) {
                $this->getResponse()->representJson($this->_objectManager->create('Magento\Framework\Json\Helper\Data')->jsonEncode($e->getMessage()));
            }
        } else {
            $this->getResponse()->representJson($this->_objectManager->create('Magento\Framework\Json\Helper\Data')->jsonEncode(""));
        }
    }
}
