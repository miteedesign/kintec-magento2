...
class UBMigrationSetup
{
    ...
    public static function createTables($installer)
    {
        ...
        $neededTableKeyNames = [
            ...
            '8', '8_review', '8_review_summary', '8_rating', '8_subscriber'
        ];
        ...
        $sql = "INSERT INTO `{$tblName}` (`title`, `code`, `status`, `setting_data`, `start_at`, `end_at`, `descriptions`, `sorder`) VALUES
                ('Databases', 'step1', 0, NULL, NULL, NULL, '- Specify databases information', 1),
                ('Websites, Stores', 'step2', 0, NULL, NULL, NULL, '- We have to migrate data from tables: core_website, core_store_group, core_store\n<br/>- Let''s select websites, store groups & store views to migration.', 2),
                ('Attributes', 'step3', 0, NULL, NULL, NULL, 'We have to migrate data from tables: eav_attribute_set, eav_attribute_group, eav_attribute, eav_attribute_label, eav_attribute_option, eav_attribute_option_value, eav_entity_attribute, catalog_eav_attribute', 3),
                ('Categories', 'step4', 0, NULL, NULL, NULL, '- We have to migrate data from tables: \ncatalog_category_entity, catalog_category_entity_datetime, catalog_category_entity_decimal, catalog_category_entity_int, catalog_category_entity_text, catalog_category_entity_text, catalog_category_entity_varchar\n<br/><br/>- Let''s select categories to migration. If you don''t specify categories, the Tool will migrate all categories.', 4),
                ('Products', 'step5', 0, NULL, NULL, NULL, '- Select product types to migrate', 5),
                ('Customers', 'step6', 0, NULL, NULL, NULL, '- Select customer groups you want to migrate <br/>- System will auto migrate the related data for each selected customer group.', 6),
                ('Sales', 'step7', 0, NULL, NULL, NULL, '- Sales Data migration <br/>- System will auto migrate the related data for each sales object', 7),
                ('Others', 'step8', 0, NULL, NULL, NULL, 'Other data objects settings', 8);";
