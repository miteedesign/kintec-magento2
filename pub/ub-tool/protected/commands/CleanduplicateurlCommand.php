...
class CleanduplicateurlCommand extends CConsoleCommand
{
    public function actionIndex()
    {
        ...
        $attributeIds = [];
        $entityTypeId = UBMigrate::getMage2EntityTypeId(UBMigrate::PRODUCT_TYPE_CODE);
        ...
        if ($attributeIds) {
            ...
            foreach ($attributeIds as $attributeCode => $attributeId) {
                //get good items
                $query = "SELECT `value_id` FROM `{$tablePrefix}catalog_product_entity_varchar` WHERE `attribute_id` = {$attributeId} GROUP BY `store_id`, `value`";
                ...
                $strIds = implode(',', $ids);
                //clean all records with duplicated values
